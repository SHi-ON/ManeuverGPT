...
from common.config import ManeuverParameters, SYSTEM_PROMPT
...
def create_tasks(iteration_id: int, output_dir: str) -> list:
    ...
    enrich_query_output = os.path.join(output_dir,
                                       f"enriched_query_{iteration_id}.txt")
    create_maneuver_output = os.path.join(output_dir,
                                          f"raw_maneuver_{iteration_id}.json")
    validate_maneuver_output = os.path.join(output_dir,
                                            f"validated_maneuver_{iteration_id}.json")
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            for i in range(1, args.iterations + 1):
                iteration_output_dir = os.path.join(output_base_dir,
                                                    f"iteration_{i}")
                os.makedirs(iteration_output_dir, exist_ok=True)
                futures.append(
                    executor.submit(generate_and_enqueue, args, i,
                                    iteration_output_dir)
                )

            # Optional: Use tqdm to display progress
            for future in tqdm(as_completed(futures), total=args.iterations,
