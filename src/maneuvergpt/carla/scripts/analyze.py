...
def normalize_time(df):
    ...
    """
    df = df.copy()
    df['time'] = (df['timestamp'] - df[
        'timestamp'].min()) / 1000.0  # Convert to seconds
    return df
...
def smooth_data(mean, common_time, smoothing_factor=500):
    ...
    smooth_time = np.linspace(common_time.min(), common_time.max(),
                              smoothing_factor)
    smooth_mean = spl(smooth_time)
    return smooth_time, smooth_mean


def plot_velocity(common_time, mean_velocities, ci_velocities,
    ...
    for key in mean_velocities.keys():
        ...
        smooth_time, smooth_mean = smooth_data(mean_velocities[key],
                                               common_time)
...
def main(test_mode=False, num_files=None):
    ...
    for idx, df in enumerate(normalized_dfs, start=1):
        interpolated = interpolate_to_common_time(df, common_time,
                                                  velocity_columns)
        all_interpolated.append(interpolated)

    # Calculate mean and confidence intervals
    mean_velocities, ci_velocities = calculate_statistics(all_interpolated,
                                                          velocity_columns)
